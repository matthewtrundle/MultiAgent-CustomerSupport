...
function extractKeywords(text: string): string[] {
  ...
  const domainKeywords = {
    technical: ['calendar', 'sync', 'api', 'integration', 'error', 'broken', 'bug', 'password', 'login', 'ical', 'smart lock', 'lock', 'code', 'wifi', 'connection'],
    billing: ['payment', 'refund', 'payout', 'charge', 'fee', 'commission', 'tax', 'invoice', 'deposit', 'money', 'price', 'cost'],
    product: ['feature', 'how to', 'guide', 'setup', 'pricing', 'listing', 'property', 'amenity', 'rules', 'policy', 'availability'],
    issues: ['not working', 'broken', 'failed', 'cant', 'wont', 'problem', 'issue', 'error', 'help', 'urgent', 'asap'],
  };
  ...
  const commonWords = new Set(['the', 'and', 'but', 'for', 'with', 'this', 'that', 'have', 'has', 'had', 'can', 'cant', 'wont', 'from', 'been']);
...
function extractEntities(text: string): TextAnalysisResult['entities'] {
  ...
  // Systems
  const systems = ['airbnb', 'vrbo', 'booking.com', 'ical', 'calendar', 'pms', 'property management system', 'smart lock', 'august', 'yale'];
  ...
  // Common issues
  const issuePatterns = ['not working', 'broken', 'stopped working', 'error', 'failed', 'cant access', 'wont sync'];
  ...
  // Features
  const features = ['calendar sync', 'pricing', 'refund', 'payout', 'booking', 'guest', 'host', 'listing'];
...
function analyzeSentiment(text: string): number {
  ...
  const negativeWords = ['broken', 'not working', 'failed', 'error', 'cant', 'wont', 'problem', 'issue', 'urgent', 'frustrated', 'angry', 'terrible', 'worst', 'never'];
  const positiveWords = ['working', 'fixed', 'solved', 'great', 'excellent', 'perfect', 'love', 'amazing', 'best'];
...
function detectCategoryScores(text: string, keywords: string[]): Record<string, number> {
  ...
  const categoryKeywords = {
    TECHNICAL: {
      strong: ['api', 'integration', 'sync', 'calendar', 'ical', 'smart lock', 'error', 'bug', 'broken'],
...
function findUrgencyPatterns(text: string): string[] {
  ...
  const patterns = [
    ...
    { pattern: /guest arriving|check.?in today|arriving soon/i, indicator: 'Imminent guest arrival' },
    { pattern: /multiple properties|all my properties/i, indicator: 'Multiple properties affected' },
  ];
